...
class FileUploadConnector @Inject() (
  ...
  def closeEnvelope(envId: String)(implicit hc: HeaderCarrier): Future[String] = {
    ...
    val result = httpClientV2
      .post(url"$fileUploadUrl/file-routing/requests")
  ...
  def retry(envelopeId: String, cur: Int, attempt: Int, factor: Float = 2f)(implicit
    hc: HeaderCarrier
  ): Future[Envelope] =
    attempt match {
      case attempt: Int if attempt < maxAttemptNumber =>
        ...
        after(nextTry.milliseconds, as.scheduler, ec, () => envelopeSummary(envelopeId, nextTry, nextAttempt))
      case _                                          =>
        Future.failed(
          new RuntimeException(
            s"[FileUploadConnector][retry] envelope[$envelopeId] summary failed at attempt: $attempt"
          )
        )
    }

  def envelopeSummary(envelopeId: String, nextTry: Int = firstRetryMilliseconds, attempt: Int = 1)(implicit
    hc: HeaderCarrier
  ): Future[Envelope] =
    httpClientV2.get(url"$fileUploadUrl/file-upload/envelopes/$envelopeId").execute[HttpResponse].flatMap { response =>
      response.status match {
        case OK        =>
        ...
        case _         =>
          Future.failed(
            new RuntimeException(s"[FileUploadConnector][envelopeSummary]Failed with status [${response.status}]")
          )
      }
    }
